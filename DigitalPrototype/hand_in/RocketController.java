...
public class RocketController extends WorldController implements ContactListener {
	...
	public void update(float dt) {
		...
		if(Gdx.input.isKeyPressed(Input.Keys.UP)) rocket.setFY(rocket.getThrust());
		else rocket.setFY(0);

		if(Gdx.input.isKeyPressed(Input.Keys.LEFT)) rocket.setFX(-rocket.getThrust());
		if(Gdx.input.isKeyPressed(Input.Keys.RIGHT)) rocket.setFX(rocket.getThrust());
