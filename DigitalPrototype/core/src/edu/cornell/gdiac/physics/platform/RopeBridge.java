...
public class RopeBridge extends ComplexObstacle {
	...
	protected boolean createJoints(World world) {
		...
		for (int ii = 0; ii < bodies.size-1; ii++) {
			...
			// Definition for a revolute joint
			RevoluteJointDef tempJointDef = new RevoluteJointDef();

			// Initial joint
			tempJointDef.bodyA = bodies.get(ii).getBody();
			tempJointDef.bodyB = bodies.get(ii+1).getBody();
			tempJointDef.localAnchorA.set(anchor1);
			tempJointDef.localAnchorB.set(anchor2);
			tempJointDef.collideConnected = false;
			Joint tempJoint = world.createJoint(tempJointDef);
